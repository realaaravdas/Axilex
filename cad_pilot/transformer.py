...
class CadTransformer(Transformer):
    ...
    def translate(self, args):
        ...
        # Collect all objects generated by nested statements
        nested_objects = []
        for stmt in statements:
            # Temporarily clear object stack to isolate nested statement results
            temp_stack = list(self.object_stack)
            self.object_stack = []
            self.transform(stmt)
            # Objects from this statement are now on the (temporarily cleared) stack
            nested_objects.extend(self.object_stack)
            self.object_stack = temp_stack # Restore original stack
        
        # Apply transformation to collected objects and push them back
        for obj in nested_objects:
            if isinstance(obj, Shape):
                obj.translate(x, y, z)
                self._push_object(obj) # Push transformed object back
        return None
    ...
    def rotate(self, args):
        ...
        for obj in nested_objects:
            if isinstance(obj, Shape):
                obj.rotate(angle, axis[0], axis[1], axis[2]) # Pass individual components
    ...
    def union(self, args):
        ...
        for stmt in args:
            ...
            self.object_stack = []
            self.transform(stmt)
            # After transform, the result of stmt is on the (temp) stack
